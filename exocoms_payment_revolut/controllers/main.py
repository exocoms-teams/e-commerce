...
class RevolutController(http.Controller):
    ...
    def revolut_return_from_checkout(self, **data):
        ...
        if tx_sudo:
            self._fetch_and_process_order(tx_sudo, tx_sudo.provider_reference)
    ...
    def revolut_webhook(self):
        ...
        self._fetch_and_process_order(provider_sudo, order_id)
        return ''  # Acknowledge the notification.

    @staticmethod
    def _fetch_and_process_order(record_sudo, order_id):
        """Fetch the order from Revolut and process it as the source of truth.
        ...
        request.env['payment.transaction'].sudo()._process('revolut', order_data)
