...
class SaleOrder(models.Model):
    ...
    is_mandat_administratif = fields.Boolean(string='Est un mandat administratif', store=True)
    ...
    imputation_ids = fields.One2many('mandat.imputation', 'sale_order_id', 'Imputations budgétaires')
    ...
    date_limite_paiement      = fields.Date('Date limite paiement', compute='_compute_dlp', store=True)
    taux_interet_moratoire    = fields.Float('Taux IM (%)', default=3.5)
    montant_interet_moratoire = fields.Monetary('Intérêts moratoires', compute='_compute_im', store=True, currency_field='currency_id')
    ...
    montant_fctva = fields.Monetary('Montant FCTVA', compute='_compute_fctva', store=True, currency_field='currency_id')
    ...
    def _compute_dlp(self):
        for o in self:
            if o.date_order and o.delai_paiement_jours:
                o.date_limite_paiement = o.date_order.date() + timedelta(days=o.delai_paiement_jours)
            else:
    ...
    def _compute_im(self):
        ...
        for o in self:
            if o.date_limite_paiement and o.mandat_state not in ('paye', 'annule') and today > o.date_limite_paiement:
                jours = (today - o.date_limite_paiement).days
                o.montant_interet_moratoire = o.amount_total * (o.taux_interet_moratoire / 100) * jours / 365
            else:
    ...
    def _compute_fctva(self):
        for o in self:
            o.montant_fctva = o.amount_tax * (o.taux_fctva / 100) if o.regime_tva_public == 'fctva' else 0.0
    ...
    def _check_siret(self):
        for o in self:
            if o.acheteur_siret and not luhn_ok(o.acheteur_siret):
                raise ValidationError(_('SIRET invalide : %s (vérification Luhn échouée).') % o.acheteur_siret)
    ...
    def action_generer_pj_std(self):
        ...
        for tp, lib, obl in types:
            if tp not in existing:
                self.env['mandat.pj'].create({'sale_order_id': self.id, 'type_pj': tp, 'libelle': lib, 'obligatoire': obl})
    ...
    def action_export_facturx(self):
        ...
        self.write({'statut_chorus': 'depose', 'date_envoi_chorus': fields.Date.today(), 'chorus_envoye': True})
        return {'type': 'ir.actions.act_url', 'url': '/web/content/%d?download=true' % att.id, 'target': 'self'}
    ...
    def _build_ubl_xml(self):
        ...
        etree.SubElement(root, cbc('CustomizationID')).text = 'urn:cen.eu:en16931:2017#compliant#urn:fdc:peppol.eu:2017:poacc:billing:3.0'
        etree.SubElement(root, cbc('ProfileID')).text = 'urn:fdc:peppol.eu:2017:poacc:billing:01:1.0'
        ...
        etree.SubElement(tt, cbc('TaxAmount'), currencyID=self.currency_id.name).text = str(round(self.amount_tax, 2))
        ...
        for i, line in enumerate(self.order_line, 1):
            ...
            etree.SubElement(il, cbc('InvoicedQuantity'), unitCode=line.product_uom_id.name or 'C62').text = str(line.product_uom_qty)
            etree.SubElement(il, cbc('LineExtensionAmount'), currencyID=self.currency_id.name).text = str(round(line.price_subtotal, 2))
            ...
            etree.SubElement(pr, cbc('PriceAmount'), currencyID=self.currency_id.name).text = str(round(line.price_unit, 2))
    ...
    def _prepare_invoice(self):
        ...
        vals.update({
            'engagement_juridique': self.numero_engagement,
            'chorus_service_code': self.service_chorus,
        })
        return vals
    ...
    def _onchange_partner_mandat(self):
        ...
        if p and p.is_public_entity:
            ...
            self.service_chorus          = p.chorus_service_code or ''
