/** @odoo-module **/
console.log('[mandat_administratif] JS chargé ✓');
...
document.addEventListener('click', async function (e) {
    ...
    try {
        ...
        console.log('[mandat_administratif] réponse serveur:', JSON.stringify(data));
        if (data?.result?.success) {
            ...
            console.error('[mandat_administratif] Erreur:', msg);
            if (errorDiv) {
        ...
        console.error('[mandat_administratif] Fetch error:', err);
        btn.disabled = false;
        btn.textContent = '🏛 Confirmer le paiement par Mandat Administratif';
    }
});
