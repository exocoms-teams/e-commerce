...
class PaymentProviderMandat(models.Model):
    ...
    def _get_compatible_providers(self, company_id, partner_id, amount, **kwargs):
        """Afficher le mandat administratif uniquement aux organismes publics."""
        providers = super()._get_compatible_providers(company_id, partner_id, amount, **kwargs)
        partner = self.env['res.partner'].browse(partner_id).exists()
        is_public = partner and (
            partner.is_organisme_public
            or partner.commercial_partner_id.is_organisme_public
        )
        if not is_public:
