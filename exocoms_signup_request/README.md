...
La demande enregistre le site et la société d'origine. Le lien de confirmation
est construit à partir du domaine réel d'inscription (`website.domain`, repli
`url_root`), pas du `web.base.url` global. Vérifier que le champ `domain` de
chaque site est renseigné.
...
* `//app[@name='base_setup']` sur la vue des réglages.
