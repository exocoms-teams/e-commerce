...
class ExocomsSignupRequest(models.Model):
    ...
    _token_uniq = models.Constraint('unique(token)', "Jeton déjà utilisé.")

    # ------------------------------------------------------------------
    # Paramétrage
    # ------------------------------------------------------------------
    # Surcharges disponibles par société. Une société qui n'active pas sa
    # propre politique hérite intégralement des réglages généraux.
    COMPANY_INT_OVERRIDES = {
        'token_ttl_hours': 'exocoms_token_ttl_hours',
        'max_per_ip': 'exocoms_max_per_ip',
    }
    COMPANY_BOOL_OVERRIDES = {
        'block_disposable': 'exocoms_block_disposable',
        'check_mx': 'exocoms_check_mx',
        'restrict_allowed': 'exocoms_restrict_allowed',
    }

    @api.model
    def _get_int_param(self, key, company=None):
        if company and key in self.COMPANY_INT_OVERRIDES:
            override = company._exocoms_int_override(self.COMPANY_INT_OVERRIDES[key])
    ...
    @api.model
    def _get_bool_param(self, key, default=True, company=None):
        if company and key in self.COMPANY_BOOL_OVERRIDES:
            override = company._exocoms_bool_override(self.COMPANY_BOOL_OVERRIDES[key])
            if override is not None:
                return override
    ...
    # ------------------------------------------------------------------
    @api.model
    def _check_email(self, email, company=None):
        ...
        if self._get_bool_param('block_disposable', True, company=company):
            if domain in Domains._exocoms_domain_set('blocked', company=company):
                raise UserError(_(
                    "Les adresses email jetables ne sont pas acceptées. "
                    "Merci d'utiliser une adresse professionnelle ou "
                    "personnelle permanente."))

        if self._get_bool_param('restrict_allowed', False, company=company):
            allowed = Domains._exocoms_domain_set('allowed', company=company)
            if allowed and domain not in allowed:
                raise UserError(_(
                    "Les inscriptions sont réservées à une liste de domaines "
                    "autorisés. Merci de contacter notre support."))

        if self._get_bool_param('check_mx', True, company=company):
    ...
    @api.model
    def _check_source_quota(self, ip_hash, company=None):
        ...
        if not ip_hash:
            return True
        max_per_ip = self._get_int_param('max_per_ip', company=company)
    ...
    @api.model
    def _find_pending(self, email, company=None):
        """Demande en attente pour une adresse, éventuellement par société.

        La portée par société est essentielle en multi-site : un client ayant
        une demande en cours sur le site A doit pouvoir en déposer une pour le
        site B, faute de quoi il ne serait jamais rattaché à la seconde
        société.
        """
        if not email:
            return self.browse()
        domain = [('email', '=ilike', self._ilike_escape(email.strip())),
                  ('state', '=', 'pending')]
        if company:
    ...
    def _submit(self, email, name=None, base_url=None, website=None, lang=None,
        ...
        self._check_source_quota(ip_hash, company=company)

        try:
            address = self._check_email(email, company=company)
        ...
        pending = self._find_pending(address, company=company)
        if pending:
    ...
    @api.model
    def _log_rejection(self, email, reason, ip_hash=False, company=None):
        ...
        self.sudo().write({
            ...
            'expiry': fields.Datetime.now() + timedelta(
                hours=self._get_int_param(
                    'token_ttl_hours', company=self.company_id)),
            'state': 'pending',
        })
        return token
