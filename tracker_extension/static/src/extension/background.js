...
async function handleProductDetected(product, isPurchase, source) {
  ...
  try {
    ...
    if (isPurchase) entry.purchaseCount += 1;
    ...
    if (parsedRating !== null) {
      ...
      if (entry.ratings.length > 20) entry.ratings = entry.ratings.slice(-20);
    }
...
async function getData() {
  try {
    ...
    return { products: [], topProducts: [], categories: {}, domains: {}, daily: {}, lastUpdated: null, totalProducts: 0, totalViews: 0, totalPurchases: 0 };
  }
}
