...
window.gm_authFailure = function() {
    ...
    if (el) el.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100%;background:#FEF2F2;color:#991B1B;font-size:14px;text-align:center;padding:20px">❌ Clé API invalide ou domaine non autorisé</div>';
};
...
window.CarteMap = (() => {
    ...
    function onGoogleMapsReady() {
        ...
        // Si la vue carte est déjà active, initialiser
    ...
    function init() {
        ...
        if (!_ready) {
            _setText('googleMap', 'Google Maps en cours de chargement…');
            // Réessayer
            var n = 0;
            var t = setInterval(function() {
                n++;
                if (_ready) { clearInterval(t); setTimeout(_doInit, 100); }
                if (n > 20)  { clearInterval(t); _setText('googleMap', '⚠️ Google Maps non disponible'); }
            }, 500);
            return;
        }
        if (_map) {
            // Déjà initialisé : resize + reload
    ...
    function _doInit() {
        ...
        console.log('[CarteMap] _doInit — container size:', el.offsetWidth + 'x' + el.offsetHeight);
        ...
        _distSvc  = new google.maps.DistanceMatrixService();
        ...
        // Resize après 300ms pour s'assurer que la carte remplit le div
        setTimeout(function() {
            google.maps.event.trigger(_map, 'resize');
        }, 300);
    ...
    function _onPos(pos) {
        ...
        // Géocodage inverse
        if (_geocoder) {
            _geocoder.geocode({ location: _userPos }, function(res, st) {
                if (st === 'OK' && res[0]) {
                    ...
                    _setText('carteCityName', city);
                    _setText('carteAddrName', num ? num + ' ' + route : route || res[0].formatted_address);
                    _setText('carteSubtitle', 'Zone d\'intervention · GPS actif');
    ...
    async function _loadMissions() {
        try {
            ...
            _missions = (data.missions || []).filter(function(m) {
                return ['assigne','rdv_planifie','en_cours','devis_envoye','devis_accepte','travaux_en_cours','nouveau'].includes(m.state);
            });
        } catch(e) {
            try { _missions = JSON.parse(localStorage.getItem('ss_missions_cache') || '[]'); } catch(e2) {}
        }
        ...
        // Géocoder chaque mission
        _missions.forEach(function(mission) {
            var addr = mission.adresse_intervention || mission.adresse;
            if (!addr || !_geocoder) return;
            _geocoder.geocode({ address: addr + ', France' }, function(res, st) {
    ...
    function _select(mission, pos) {
        ...
        _setText('carteInfoTitle', mission.description_sinistre || mission.description || '—');
        _setText('carteInfoAddr',  mission.adresse_intervention || mission.adresse || '—');
        _setText('carteInfoMontant', mission.montant ? mission.montant.toLocaleString('fr-FR') + ' €' : '—');
        _setText('carteInfoDuree', _userPos ? 'Calcul…' : '—');
        _setText('carteInfoDist',  '—');

        _map.panTo(pos);
        _map.setZoom(15);

        if (_userPos && _distSvc) {
            _distSvc.getDistanceMatrix({
                origins: [_userPos],
                ...
                if (st === 'OK') {
                    ...
                    if (el.status === 'OK') {
                        _setText('carteInfoDuree', el.duration_in_traffic ? el.duration_in_traffic.text : el.duration.text);
                        _setText('carteInfoDist',  el.distance.text);
                    }
                }
            });
        }
    ...
    function _updateDistances() {
        if (!_userPos || !_markers.length || !_distSvc) return;
    ...
    function _updateNext() {
        ...
        if (!el) return;
        var first = _markers[0] || null;
        var m = first ? first.mission : (_missions[0] || null);
        ...
        el.innerHTML = '<div style="font-size:14px;font-weight:700;margin-bottom:4px">' + (m.description_sinistre || '—') + '</div>' +
            ...
            (first && first.distTxt ? '<div style="font-size:12px;color:#374151;margin-bottom:10px">🚗 ' + first.durTxt + ' · 📍 ' + first.distTxt + '</div>' : '') +
    ...
    function _updateProches() {
        ...
        el.innerHTML = _markers.slice(0, 5).map(function(mm) {
            var m = mm.mission;
            var color = COLORS[m.type_intervention] || '#6B7280';
    ...
    function launchRoute() {
        var m = _selected;
        if (!m) return;
        var dest = encodeURIComponent((m.adresse_intervention || m.adresse || '') + ', France');
        ...
        if (_userPos && _map) {
            new google.maps.DirectionsService().route({
                origin: _userPos,
                destination: (m.adresse_intervention || m.adresse || '') + ', France',
                travelMode: google.maps.TravelMode.DRIVING,
                drivingOptions: { departureTime: new Date(), trafficModel: 'bestguess' },
            }, function(res, st) {
                if (st === 'OK') _directionsRenderer.setDirections(res);
            });
        }

        // Ouvrir Google Maps natif
    ...
    function openMission() {
        ...
        closeInfoPanel();
        if (window.MissionDetail) MissionDetail.open(_selected.id);
    }

    function closeInfoPanel() {
        var p = document.getElementById('carteInfoPanel');
        if (p) p.style.display = 'none';
    }

    function centerOnUser() {
        if (_userPos && _map) { _map.panTo(_userPos); _map.setZoom(15); }
        else if (window.Toast) Toast.show('Position GPS non disponible', 'warning');
    }
    ...
    function _selectById(id) {
        var mm = _markers.find(function(m) { return String(m.mission.id) === String(id); });
        if (mm) _select(mm.mission, mm.pos);
    }
    ...
    function _icon(color, urgent) {
        var svg = '<svg xmlns="http://www.w3.org/2000/svg" width="36" height="44" viewBox="0 0 36 44">'
            ...
            + '<line x1="18" y1="34" x2="18" y2="44" stroke="' + color + '" stroke-width="3"/>'
            + '</svg>';
        return {
