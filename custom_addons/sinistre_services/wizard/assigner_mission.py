...
class AssignerMissionWizard(models.TransientModel):
    ...
    mission_ids = fields.Many2many('sinistre.mission', string='Missions',
                                   default=lambda self: self.env.context.get('active_ids', []))
    intervenant_id = fields.Many2one('sinistre.intervenant', string='Intervenant', required=True,
    ...
    def action_assigner(self):
        ...
        for m in self.mission_ids:
            m.write({'intervenant_id': self.intervenant_id.id, 'date_rdv': self.date_rdv, 'state': 'rdv_planifie'})
            m.message_post(
