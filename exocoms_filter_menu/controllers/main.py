...
class ExoFilterSidebar(http.Controller):
    ...
    def facets(self, **kw):
        ...
        # has_active avec garde anti-cycles via visited
...
class ExoShopController(WebsiteSale):
    """
    Surcharge du controller /shop natif d'Odoo pour injecter
    la sidebar EXOCOMS dans le contexte de rendu.
    Le template exo_shop_layout hérite de website_sale.products
    pour remplacer la grille par #exo-products.
    """
    ...
    def shop(self, page=0, category=None, search='', min_price=0.0,
        ...
        # Appel du parent pour récupérer la réponse native
        response = super().shop(
            page=page, category=category, search=search,
            min_price=min_price, max_price=max_price, ppg=ppg, **post
        )
        # Marquer le contexte pour que le template puisse injecter la sidebar
